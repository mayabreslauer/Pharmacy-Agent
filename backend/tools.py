...
TOOLS = [
    ...
    # ===== INVENTORY CONTROL =====
        ...
        "function": {
            ...
            "description": "When user asks to reserve medication for pickup. Only when user_id is available.",
            "parameters": {
                "type": "object",
                "properties": {
                    "medication_name": {
                    ...
                    "user_id": {
                        "type": "string",
                        "description": "User ID from conversation context (REQUIRED - only call this tool if user_id exists)"
                    }
                },
                "required": ["medication_name", "quantity"],  # ✅ user_id לא required כדי שה-LLM לא ימציא אותו!
                "additionalProperties": False
            }
        }
    },
    # ===== CUSTOMER SERVICE =====
    {
        ...
        "function": {
            ...
            "description": "When user asks if medications are safe to take together. Only when user_id is available.",
            "parameters": {
                "type": "object",
                "properties": {
                    "medications": {
...
def execute_reserve_medication(medication_name: str, quantity: int, user_id: str = None) -> Dict[str, Any]:
    """
    Execute: Reserve medication for pickup
    ✅ user_id is now optional - if not provided, returns error
    """
    # ✅ בדיקה אם יש user_id
    if not user_id or not user_id.strip():
        return {
            "success": False,
            "error": "User identification required for reservations. Please provide your customer ID or visit the pharmacy in person."
        }
    ...
    return {
        ...
        "quantity": quantity,
        "user_id": user_id,
        "message": f"Reservation confirmed for user {user_id}. Please pick up within 48 hours. Reservation ID: {reservation_id}"
    }
